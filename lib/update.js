var q = require('q'),
app = require('flatiron').app,
path = require('path'),
child_process = require('child_process');

function run(command) {
	return q.nfcall(
		child_process.exec,
		command,
		{ cwd: path.join(__dirname, '..') }
	);
}

function fetch() {
	return run('git status --porcelain')
	.then(function(stdout) {
		app.log.verbose.apply(app.log, stdout);
		return run('git fetch');
	});
}

function checkout(ref) {
	app.log.verbose('Checking out', ref);
	return run('git checkout '+ref);
}

function diff(ref) {
	return run('git diff --name-only ..' + ref)
	.then(function(stdout) {
		app.log.verbose.apply(app.log, stdout);
		// Affects the application
		return stdout && stdout.some(function(file) {
			return /^lib\/|^plugins\/|^[^\/]+$/.test(file);
		});
	});
}

function rebuild() {
	app.log.info('Rebuilding Wintersmith');
	return q.ninvoke(app.wintersmith, 'build');
}

module.exports = function() {
	this.post(function() {
		var body = this.req.body,
		send = this.res,
		ref, appChanged;

		if (!(body.repository && 'refs/heads/'+body.repository.master_branch === body.ref)) {
			this.res.json(this.req.body);
			return;
		}

		app.log.info('Received hook for', body.repository.name);

		ref = body.head_commit.id;
		fetch()
		.then(function() {
			return diff(ref);
		})
		.then(function(restart) {
			appChanged = restart;
			var chain = checkout(ref);
			return restart ? chain : chain.then(rebuild);
		})
		.then(function(res) {
			send.json(res);
		}, function(err) {
			send.json(err);
		})
		.then(function() {
			if (appChanged) {
				process.exit(0);
			}
		});
	});
};
